...
/***/ (function(module, exports, __webpack_require__) {
	...
	var recipeEvents = __webpack_require__(12);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var onGetRecipes = function onGetRecipes(event) {
	  event.preventDefault();
	  api.index().then(ui.onGetRecipesSuccess).catch(ui.onGetRecipesFailure);
	};

	var onGetRecipe = function onGetRecipe(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.show(data).then(ui.onGetRecipeSuccess).catch(ui.onGetRecipeFailure);
	};

	var onShowUpdateForm = function onShowUpdateForm(event) {
	  var recipeID = event.relatedTarget.parentNode.getAttribute('data-id');

	  var currentRow = $(event.relatedTarget).closest('tr');
	  var name = currentRow.find('.rname').text();
	  var location = currentRow.find('.rlocation').text();
	  var comment = currentRow.find('.rcomment').text();

	  var form = $('#update-recipe');
	...
	var onUpdateRecipe = function onUpdateRecipe(event) {
	  event.preventDefault();
	  var form = $('#update-recipe');
	  var recipeID = form.find('.urid').val();
	  var data = getFormFields(event.target);
	  api.update(data, recipeID).then(function (response) {
	    // console.log('onUpdateRecipe')
	    // console.log(response)
	  }).then(ui.onUpdateRecipeSuccess).catch(ui.onUpdateRecipeFailure);
	};

	var onCreateRecipe = function onCreateRecipe(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.create(data).then(ui.onCreateRecipeSuccess).catch(ui.onCreateRecipeFailure);
	};

	var onDeleteRecipe = function onDeleteRecipe(event) {
	  event.preventDefault();
	  var recipeID = event.currentTarget.parentNode.getAttribute('data-id');
	  api.destroy(recipeID).then(ui.onDestroyRecipeSuccess).catch(ui.onDestroyRecipeFailure);
	};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var destroy = function destroy(recipeID) {
	  return $.ajax({
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var showRecipesTemplate = __webpack_require__(15);

	var recipeEngine = __webpack_require__(10);

	var onGetRecipesSuccess = function onGetRecipesSuccess() {
	  $('.recipe-rows').empty();

	  var showRecipesHtml = showRecipesTemplate({ recipes: recipeEngine.recipes });
	  $('.recipe-rows').append(showRecipesHtml);
	  $('#welcome-pg').hide(500);
	  $('#recipes-table').show(500);
	};

	var onGetRecipesFailure = function onGetRecipesFailure(error) {
	  $('div#statusBar').text('Get recipes failed. Status = ' + error.status + ' ' + error.statusText);
	  $('div#statusBar').show(3000);
	  $('div#statusBar').hide(5000);
	};

	var onGetRecipeSuccess = function onGetRecipeSuccess(response) {};

	var onGetRecipeFailure = function onGetRecipeFailure(error) {
	  $('div#statusBar').text('Get recipe failed. Status = ' + error.status + ' ' + error.statusText);
	  $('div#statusBar').show(3000);
	  $('div#statusBar').hide(5000);
	};

	var onUpdateRecipeSuccess = function onUpdateRecipeSuccess(response) {
	  // $('#show-my-recipes-btn').click()
	  $('#update-recipe-close').click();
	  $('#show-my-recipes-btn').click();
	};

	var onUpdateRecipeFailure = function onUpdateRecipeFailure(error) {
	  $('div#statusBar').text('Edit recipe failed. Status = ' + error.status + ' ' + error.statusText);
	  $('div#statusBar').show(3000);
	  $('div#statusBar').hide(5000);
	};

	var onCreateRecipeSuccess = function onCreateRecipeSuccess() {
	  $('#add-recipe-close').click();
	  var recipeToArray = [];
	  recipeToArray.push(recipeEngine.recipe);
	  var showRecipesHtml = showRecipesTemplate({ recipes: recipeToArray });
	  $('.recipe-rows').append(showRecipesHtml);
	  $('#show-my-recipes-btn').click();
	};

	var onCreateRecipeFailure = function onCreateRecipeFailure(error) {
	...
	var onDestroyRecipeFailure = function onDestroyRecipeFailure(error) {
	...
	module.exports = {
	  onGetRecipesSuccess: onGetRecipesSuccess,
	  onGetRecipesFailure: onGetRecipesFailure,
	  onGetRecipeSuccess: onGetRecipeSuccess,
	  onGetRecipeFailure: onGetRecipeFailure,
	  onUpdateRecipeSuccess: onUpdateRecipeSuccess,
	  onUpdateRecipeFailure: onUpdateRecipeFailure,
	  ...
	  onDestroyRecipeFailure: onDestroyRecipeFailure
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "\n  <tr data-id=\""
	    ...
	    + "\">\n    <td data-id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\"><button id=\"editRecipe"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" class=\"btn btn-default editRecipe\" data-toggle=\"modal\" data-target=\"#updateRecipe\">Edit</button></td>\n    <td data-id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\"><button id=\"deleteRecipe"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" class=\"btn btn-default deleteRecipe\">Delete</button></td>\n    <td class='rname' name=\"[recipe][name]\">"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.name : stack1), depth0))
	    + "</td>\n    <td class='rlocation' name=\"[recipe][location]\">"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.location : stack1), depth0))
	    + "</td>\n    <td class='rcomment' name=\"[recipe][comment]\">"
